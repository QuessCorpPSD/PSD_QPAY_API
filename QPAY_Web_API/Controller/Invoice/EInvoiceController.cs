   ...
   [FromForm] string payperiodId, [FromForm] string CreatedBy)
        ...
        public async Task<IActionResult> GetConsolidateInvoiceSummary(DownloadRegister downloadRegister)
        {
            ...
            if (dt.Rows.Count > 0) //&& dt1.Rows.Count > 0)
            {
                ...
                //        var columnsum = dt.AsEnumerable().Sum(row => row.Field<double?>(column)) ?? 0;
                ...
                //    {
                //        var columnsum = dt.AsEnumerable().Sum(row => row.Field<Int64?>(column)) ?? 0;
                ...
                //}
                //var emptyColumns = dt.Columns.Cast<DataColumn>()
                //                          .Where(col => dt.AsEnumerable().All(row =>
                //                          {
                //                              var value = row[col];
                ...
                using var workbook = new XLWorkbook();
                {
                    //var ws = workbook.AddWorksheet(dt1, "PayRegister");
                    //ws.Table(0).ShowAutoFilter = false;
                    //ws.Table(0).Theme = XLTableTheme.None;
                    ...
                    //ws.Cell(1, 1).Value = downloadRegister.Company_Name;
                    //ws.Cell(1, 1).Style.Font.Bold = true;
                    //ws.Cell(2, 1).Value = string.Format("SALARY FOR THE MONTH OF {0}", downloadRegister.Pay_Period);
                    //ws.Cell(2, 1).Style.Font.Bold = true;
                    //var lastrow = ws.LastRowUsed().RowNumber();

                    //ws.Cell(lastrow, 1).Value = "Grand Total";

                    //var totalsummary = _ieinvoice.GetPayRegisterSummary(downloadRegister.Company_Id, downloadRegister.Pay_Period_Id);
                    ...
                    //            {
                    //                columnName = string.Format("SALARY FOR THE MONTH OF {0}", downloadRegister.Pay_Period);
                    //            }
