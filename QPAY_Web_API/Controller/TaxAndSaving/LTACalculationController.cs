...
namespace QPay.API.Controller.TaxAndSaving
{
    ...
    [ApiController]
    public class LTACalculationController : ControllerBase
    {
        private readonly ILTACalculationRepository _IRepository;
        private readonly IConfiguration _configuration;
        public LTACalculationController(IConfiguration configuration, ILTACalculationRepository Repository)
        {
        ...
        [HttpGet]
        [Route("GetLTABlockPayPeriod")]
        public async Task<IActionResult> GetLTABlockPayPeriod()
        {
            var response = await _IRepository.GetLTABlockPayPeriod();
            if (response.Tables[0].Rows.Count > 0)
            {
                var _outputResponse = ResponseWrapManager.ResponseWrapper(response, HttpContext);
                return Ok(_outputResponse);
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] LTACalculationRequest request)
        {
