...
namespace QPay.API.Controller.GlobalMaster
{
    ...
    {
        private readonly IBankRepository _IRepository;
        public BankController(IBankRepository IRepository)
        {
        ...
        public async Task<IActionResult> Create([FromBody] BankRequest request)
        {
            ...
            if (response.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    var _outputResponse = ResponseWrapManager.ResponseWrapper(message, HttpContext);
                    return Ok(_outputResponse);
                }
            }
            else
