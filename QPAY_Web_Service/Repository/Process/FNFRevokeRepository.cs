...
{
    public class FNFRevokeRepository : IFNFRevokeRepository
    {
        private readonly DbRepository _dbRepository;
        private readonly IConfiguration _configuration;
        public FNFRevokeRepository(DbRepository dbRepository, IConfiguration configuration)
        {
            this._dbRepository = dbRepository;
            this._configuration = configuration;
        }

        public async Task<ProcessResponse> ImportFNFRevoke(IFormFile file, [FromForm] string User)
        {
            ProcessResponse processDetails = new ProcessResponse();

            if (file != null && file.Length != 0)
            {
                var uploadsFolder = Path.Combine(_configuration["ClaimDocPath"].ToString(), "FNFRevoke_Process");
                ...
                string storeProcedure = "Proc_Upload_RevokeFNF";
                ...
                if (!string.IsNullOrWhiteSpace(res))
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(message) && message.Contains("Row(s) Revoked Successfully"))
                        {
