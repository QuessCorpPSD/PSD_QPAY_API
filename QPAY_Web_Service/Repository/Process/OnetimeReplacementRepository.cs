...
{
    public class OnetimeReplacementRepository : IOnetimeReplacementRepository
    {
        private readonly DbRepository _dbRepository;
        private readonly IConfiguration _configuration;
        public OnetimeReplacementRepository(DbRepository dbRepository, IConfiguration configuration)
        {
            this._dbRepository = dbRepository;
            this._configuration = configuration;
        }

        public async Task<DataSet> SearchDetails(SearchOnetimeReplacementRequest searchRequest)
        {
            var parameters = new Dictionary<string, object?>
            {
                ["@Company_Id"] = searchRequest.Company_id,
                ["@Employee_Id"] = searchRequest.Employee_Code,
                ["@Payfrequencyid"] = searchRequest.Pay_Frequency_Id

            };
            return _dbRepository.ExecuteStoredProcedureToDataSetAsync("sp_GetAllOneTimeReplacement", parameters, 1500);
        }

        public async Task<DataSet> ExporttoExcel(SearchOnetimeReplacementRequest exporttoExcelRequest)
        {
            var parameters = new Dictionary<string, object?>
            {
                ["@Company_Id"] = exporttoExcelRequest.Company_id,
                ["@Employee_Id"] = exporttoExcelRequest.Employee_Code,
                ["@Payfrequencyid"] = exporttoExcelRequest.Pay_Frequency_Id
            };
            return _dbRepository.ExecuteStoredProcedureToDataSetAsync("Proc_GetAllOneTimeReplacementExportToExcel", parameters, 1500);
        }

        public async Task<ProcessResponse> ImportOnetimeReplacement(IFormFile file, [FromForm] string User)
        {
            ProcessResponse processDetails = new ProcessResponse();

            if (file != null && file.Length != 0)
            {
                var uploadsFolder = Path.Combine(_configuration["ClaimDocPath"].ToString(), "OnetimeReplacement_Process");
                ...
                var newFileName = $"OnetimeReplacement_Process_{datePrefix}{extension}";
                ...
                string storeProcedure = "Proc_Upload_OneTime_Replacement";
        ...
        public async Task<DataSet> DeleteOnetimeReplacement(string One_Time_Replacement_Id, string CreatedBy)
        {
            string xmlInput = "<OneTimeReplacementDetail><OneTimeReplacement><One_Time_Replacement_Id>" + One_Time_Replacement_Id + "</One_Time_Replacement_Id></OneTimeReplacement></OneTimeReplacementDetail>";
            var parameters = new Dictionary<string, object?>
            {
                ["@xmlInput"] = xmlInput,                
                ["@xmlInputDetail"]="''",
            ...
            return _dbRepository.ExecuteStoredProcedureToDataSetAsync("sp_CreateUpdateOneTimeReplacement", parameters, 1500);
        }
    }
}
