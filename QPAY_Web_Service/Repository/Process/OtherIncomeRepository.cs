...
namespace QPay.BAL.Repository.Process
{
    public class OtherIncomeRepository : IOtherIncomeRepository
    {
        ...
        public OtherIncomeRepository(DbRepository dbRepository, IConfiguration configuration)
        {
            this._dbRepository = dbRepository;
            this._configuration = configuration;
        }

        public async Task<DataSet> SearchDetails(SearchOIRequest searchRequest)
        {
            string xml = "<Main>" + "<Company_id>" + searchRequest.Company_id + "</Company_id>" + "<Payperiod>" + searchRequest.Pay_Frequency_Id + "</Payperiod></Main>";

            var parameters = new Dictionary<string, object?>
            {
                ["@mode"] = "Search",
                ["@value1"] = searchRequest.Pay_Frequency_Id,
                ["@value2"] = xml
            };
            return _dbRepository.ExecuteStoredProcedureToDataSetAsync("sp_GetOtherIncomeData", parameters, 1500);
        }

        public async Task<ProcessResponse> ImportOtherIncome(IFormFile file, [FromForm] string User)
        {
            ProcessResponse processDetails = new ProcessResponse();

            if (file != null && file.Length != 0)
            {
                var uploadsFolder = Path.Combine(_configuration["ClaimDocPath"].ToString(), "OtherIncome_Process");
                ...
                var newFileName = $"OtherIncome_Process_{datePrefix}{extension}";
                ...
                string storeProcedure = "Upload_Otherincome_NewUI";
                var parameters = new DynamicParameters();

                parameters.Add("@CreatedBy", User);
                parameters.Add("@xml", xmlInput.Replace("Sheet1", "Table"));
        ...
        public async Task<DataSet> DeleteOtherIncome(string Other_Income_Id, string CreatedBy)
        {
            string xmlInput = "<Main><OtherIncome><Other_Income_Id>" + Other_Income_Id + "</Other_Income_Id></OtherIncome></Main>";
            var parameters = new Dictionary<string, object?>
            {
                ["@xmlInput"] = xmlInput,
                ["@Mode"] = "Delete",
