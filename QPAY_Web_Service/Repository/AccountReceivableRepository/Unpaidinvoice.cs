...
namespace QPay.BAL.Repository.AccountReceivableSer
{
    public class Unpaidinvoice : IUnpaidinvoice
    {
        ...
        public async Task<DataSet> UnpaidInvoiceExportToExcel(CommonExport1 payload)
        {
            var parameters = new Dictionary<string, object?>
            {
                ["@CompanyId"] = Convert.ToInt32(payload.companyId),
                ["@FromDate"] = payload.fromDate,
                ["@ToDate"] = payload.toDate,
                ["@Action"] = "Export",
                ["@AllEntityId"] = payload.allEntityId ?? "0"
            };
