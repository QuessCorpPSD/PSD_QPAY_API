...
namespace QPay.BAL.Repository.Invoice
{
    public class InvoiceInitiationRepository : IInvoiceInitiationRepository
    {
        ...
        public async Task<List<InitiationRequestUI>> InitiationSearch(InitiationRequestModel initiationRequestModel)
        {
            ...
            try
            {
                if (initiationRequestModel.Invoice_Billing_Type == 1)
                {
                    storeProcedure = "[dbo].[sp_GetAllInvoiceInitiateDetails2]" ?? "";
                    parameter.Add("@CompanyId", initiationRequestModel.Company_Id ?? (object)DBNull.Value);
                    parameter.Add("@PayPeriodId", initiationRequestModel.PayPeriod ?? (object)DBNull.Value);

                }
                else if (initiationRequestModel.Invoice_Billing_Type == 2)
                {
                    storeProcedure = "[dbo].[Proc_ManageMagnaGstInvoiceInitiate]" ?? "";
                    parameter.Add("@Company_Id", initiationRequestModel.Company_Id ?? (object)DBNull.Value);
                    parameter.Add("@Pay_Period_Id", initiationRequestModel.PayPeriod_Id ?? (object)DBNull.Value);
                    parameter.Add("@Created_By", initiationRequestModel.CreatedBy ?? (object)DBNull.Value);
                    parameter.Add("@Action", initiationRequestModel.ActionType ?? (object)DBNull.Value);
        ...
        {
            string storeProcedure = "[dbo].[Proc_ManageGstInvoiceInitiate_NEW]" ?? "";
            var parameter = new DynamicParameters();
