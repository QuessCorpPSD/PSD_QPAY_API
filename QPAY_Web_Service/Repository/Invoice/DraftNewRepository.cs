...
namespace QPay.BAL.Repository.Invoice
{
    public class DraftNewRepository : IDraftNewRepository
    {
        ...
        public async Task<InvoiceResponse> PerformaInvoiceSkip(DraftInvoiceInitiate request)
        {
            InvoiceResponse invoiceDetails = new InvoiceResponse();

            string xml = XmlHelper.SerializeObjectToXml(request.DraftInvoiceInitiateRequest);
        ...
        public async Task<List<InvoiceInitiateRequest>> GetDraftInformation(int CompanyId, int PayPriod, string createdBy)
        {
            var parameters = new Dictionary<string, object?>
            {
                ["@CompanyId"] = CompanyId,
